...
void fill_square(render_t* handle,const view_position_t start_point,const int square_size)
{
	...
	ordinal_number_t ll;
	ordinal_number_t lr;
	ordinal_number_t middle_iteration;
	...
	/* Calculate the middle od the iterations. */
	middle_iteration=(ul+ur+ll+lr)/4;

	/* Fill the square with color. */
	square_help.width=square_size;
	square_help.height=square_size;
	(*handle->output_facility->facility.output.fill_rect_function)(handle->output,start_point,square_help,middle_iteration);
}
